...
int testWWWWWWWcreate(){
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_CREATE_PRODUCT*(MAX+1);
	
	printf("*************************************** \n");
	if(end-start > expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST WWWWWW1 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST WWWWWW1 ERR ********* \n");
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********** END TEST WWWWWW1 ********* \n");
	printf("*************************************** \n");
...
int testWWWWWWdelete(){
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_DELETE_PRODUCT*(MAX+1);
	
	printf("*************************************** \n");
	if(end-start > expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST WWWWWW2 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST WWWWWW2 ERR ********* \n");
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********** END TEST WWWWWW2 ********* \n");
	printf("*************************************** \n");
...
int testRRRRRR(){
	...
	// Create 6 threads for operations over the warehouse
	for (i=0;i<MAX+1;i++){
		sprintf(product[i], "product%d", i);
		pthread_create (&th_test[i], NULL, count_products_thread, product[i]);
	}
	...
	// The 6 read operations should be don in parallel
	// (the time of 6 reads should be similar to 1)
	expected_time = TIME_COUNT_PRODUCTS*2;
	
	printf("*************************************** \n");
	if(end-start > expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST RRRRRR OK! ********** \n");
		res = TRUE;
	}else{
		printf("********** TEST RRRRRR ERR ********** \n");
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********** END TEST RRRRRR ********** \n");
	printf("*************************************** \n");
...
int testWWWWRRdelete(){
	...
	printf("*************************************** \n");
	if(end-start > expected_time && end-start < expected_time+2
*THRESHOLD){
...
int testW0W1W2W3W4create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4;
	void *st1;
	char product0[128], product1[128], product2[128], product3[128], product4[128];
	struct timeval start_time,end_time;
	struct update_stock_st stock0, stock1, stock2, stock3, stock4;
	double start,end;
	int res;
	...
	// Create 5 threads for operations over the warehouse
	sprintf(product0, "product%d", 0);
	strcpy((stock0.product),product0);
	stock0.stock=200;

	sprintf(product1, "product%d", 1);
	strcpy((stock1.product),product1);
	stock1.stock=150;

	sprintf(product2, "product%d", 2);
	strcpy((stock2.product),product2);
	stock2.stock=100;

	sprintf(product3, "product%d", 3);
	strcpy((stock3.product),product3);
	stock3.stock=75;

	sprintf(product4, "product%d", 4);
	strcpy((stock4.product),product4);
	stock4.stock=50;

	
	pthread_create (&th_test0, NULL, increment_stock_thread, &stock0);
	pthread_create (&th_test1, NULL, increment_stock_thread, &stock1);
	pthread_create (&th_test2, NULL, increment_stock_thread, &stock2);
	pthread_create (&th_test3, NULL, increment_stock_thread, &stock3);
	pthread_create (&th_test4, NULL, increment_stock_thread, &stock4);
	
	pthread_join (th_test0, (void **) &st1);
	pthread_join (th_test1, (void **) &st1);
	pthread_join (th_test2, (void **) &st1);
	pthread_join (th_test3, (void **) &st1);
	pthread_join (th_test4, (void **) &st1);
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_INCREMENT_STOCK;
	
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST W0W1W2W3W4 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST W0W1W2W3W4 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********** END TEST W0W1W2W3W4 ********* \n");
	printf("*************************************** \n");
	
	return res;
}

int testW0W0W0W0W0create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4;
	void *st1;
	char product0[128], product1[128], product2[128], product3[128], product4[128];
	struct timeval start_time,end_time;
	...
	sprintf(product1, "product%d", 0);
	strcpy((stock1.product),product0);
	stock1.stock=11;

	sprintf(product2, "product%d", 0);
	strcpy((stock2.product),product2);
	stock2.stock=10;

	sprintf(product3, "product%d", 0);
	strcpy((stock3.product),product3);
	stock3.stock=41;

	sprintf(product4, "product%d", 0);
	strcpy((stock4.product),product4);
	stock4.stock=30;

	
	pthread_create (&th_test0, NULL, decrement_stock_thread, &stock0);
	pthread_create (&th_test1, NULL, decrement_stock_thread, &stock1);
	pthread_create (&th_test2, NULL, decrement_stock_thread, &stock2);
	pthread_create (&th_test3, NULL, decrement_stock_thread, &stock3);
	pthread_create (&th_test4, NULL, decrement_stock_thread, &stock4);
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_INCREMENT_STOCK*5;
	
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST W0W0W0W0W0 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST W0W0W0W0W0 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********* END TEST W0W0W0W0W0 ********* \n");
	printf("*************************************** \n");
	
	return res;
}

int testR0R0R0R0R0R0create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4, th_test5;
	void *st1;
	char product0[128];
	struct timeval start_time,end_time;
	double start,end;
	...
	pthread_create (&th_test0, NULL, get_stock_thread, product0);
	pthread_create (&th_test1, NULL, get_stock_thread, product0);
	pthread_create (&th_test2, NULL, get_stock_thread, product0);
	pthread_create (&th_test3, NULL, get_stock_thread, product0);
	pthread_create (&th_test4, NULL, get_stock_thread, product0);
	pthread_create (&th_test5, NULL, get_stock_thread, product0);
	...
	pthread_join (th_test3, (void **) &st1);
	pthread_join (th_test4, (void **) &st1);
	pthread_join (th_test5, (void **) &st1);
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_GET_STOCK*2;
	
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST R0R0R0R0R0R0 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST R0R0R0R0R0R0 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********* END TEST R0R0R0R0R0R0 ********* \n");
	printf("*************************************** \n");
...
int testR0R1R2R3R4R5create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4, th_test5;
	void *st1;
	char product0[128], product1[128], product2[128], product3[128], product4[128], product5[128];
	struct timeval start_time,end_time;
	double start,end;
	int res;
	double expected_time;
	...
	sprintf(product3, "product%d", 3);
	sprintf(product4, "product%d", 4);
	sprintf(product5, "product%d", 5);
	
	pthread_create (&th_test0, NULL, get_stock_thread, product0);
	pthread_create (&th_test1, NULL, get_stock_thread, product1);
	pthread_create (&th_test2, NULL, get_stock_thread, product2);
	pthread_create (&th_test3, NULL, get_stock_thread, product3);
	pthread_create (&th_test4, NULL, get_stock_thread, product4);
	pthread_create (&th_test5, NULL, get_stock_thread, product5);
	...
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST R0R1R2R3R4R5 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST R0R1R2R3R4R5 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********* END TEST R0R1R2R3R4R5 ********* \n");
	printf("*************************************** \n");
...
int testW0W1W2W3R0R1create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4, th_test5;
	void *st1;
	char product0[128], product1[128], product2[128], product3[128], product4[128], product5[128];
	struct timeval start_time,end_time;
	struct update_stock_st stock0, stock1, stock2, stock3;
	double start,end;
	int res;
	double expected_time;
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_INCREMENT_STOCK+TIME_GET_STOCK;
	
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST W0W1W2W3R0R1 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST W0W1W2W3R0R1 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********* END TEST W0W1W2W3R0R1 ********* \n");
	printf("*************************************** \n");
	
	return res;
}

int testW0W0W0W0R0R0create(){
	pthread_t th_test0, th_test1, th_test2, th_test3, th_test4, th_test5;
	void *st1;
	char product0[128], product1[128], product2[128], product3[128], product4[128], product5[128];
	struct timeval start_time,end_time;
	...
	// Calculate expected time: 5 writes should be sequential
	expected_time = TIME_INCREMENT_STOCK*4+TIME_GET_STOCK;
	
	printf("*************************************** \n");
	if(end-start >= expected_time && end-start < expected_time+THRESHOLD){
		printf("********** TEST W0W0W0W0R0R0 OK! ********* \n");
		res = TRUE;
	}else{
		printf("********** TEST W0W0W0W0R0R0 ERR ********* %f %f  \n",expected_time,end-start);
		res = FALSE;
	}
	printf("*************************************** \n");
	printf("********* END TEST W0W0W0W0R0R0 ********* \n");
	printf("*************************************** \n");
...
int main(){
	...
	testWWWWWWWcreate();
	testRRRRRR();
	testW0W1W2W3W4create();
	testW0W0W0W0W0create();
	testR0R0R0R0R0R0create();
	testR0R1R2R3R4R5create();
	testW0W0W0W0R0R0create();
	testW0W1W2W3R0R1create();
	testWWWWWWdelete();
